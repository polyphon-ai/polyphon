...
const api = {
  ...
  session: {
    ...
    export: (sessionId: string, format: 'markdown' | 'json' | 'plaintext', savePath?: string): Promise<{ ok: boolean; error?: string }> =>
      ipcRenderer.invoke(IPC.SESSION_EXPORT, sessionId, format, savePath),
  ...
  logs: {
    ...
    getDebugEnabled: (): Promise<boolean> =>
      ipcRenderer.invoke(IPC.LOGS_GET_DEBUG_ENABLED),
    setDebugEnabled: (enabled: boolean): Promise<void> =>
      ipcRenderer.invoke(IPC.LOGS_SET_DEBUG_ENABLED, enabled),

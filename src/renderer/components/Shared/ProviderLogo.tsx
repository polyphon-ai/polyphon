...
export interface ProviderLogoProps {
  provider: string;
  size?: number;
  /** Override fill with a single color (e.g. "white"). */
  color?: string;
  /** Letter to show in the fallback circle when no logo exists for the provider. */
  fallbackInitial?: string;
  /** Accessible label. When provided, the SVG gets role="img" and aria-label; omitting hides it from assistive tech. */
  title?: string;
}

function AnthropicLogo({ size, color, title }: { size: number; color?: string; title?: string }) {
  return (
    <svg
      width={size}
      height={size}
...
function OpenAILogo({ size, color, title }: { size: number; color?: string; title?: string }) {
  return (
    <svg
      width={size}
      height={size}
...
function GeminiLogo({ size, color, title }: { size: number; color?: string; title?: string }) {
  return (
    <svg
      width={size}
      height={size}
...
function CopilotLogo({ size, color, title }: { size: number; color?: string; title?: string }) {
  return (
    <svg
      width={size}
      height={size}
...
export default function ProviderLogo({ provider, size = 20, color, fallbackInitial, title }: ProviderLogoProps): React.JSX.Element {
  ...
  if (!Logo) {
    const initial = fallbackInitial ?? provider.charAt(0).toUpperCase();
    return (
      <svg
        width={size}
        height={size}
      ...
      >
        {title && <title>{title}</title>}
        <circle cx="12" cy="12" r="10" fill={color ?? '#6366f1'} opacity="0.2" />
        <text x="12" y="16" textAnchor="middle" fontSize="12" fontWeight="bold" fill={color ?? '#6366f1'}>
          {initial}
        </text>
      </svg>
    );
  }
  return <Logo size={size} color={color} title={title} />;
}

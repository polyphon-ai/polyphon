...
function stubPolyphonSettings(overrides: Record<string, unknown> = {}) {
  vi.stubGlobal('polyphon', {
    settings: {
      ...
      saveProviderConfig: vi.fn().mockResolvedValue({ provider: 'anthropic', enabled: true, voiceType: 'api', defaultModel: 'claude-opus-4-6', cliCommand: null, cliArgs: null, yoloMode: false, id: 'pc-1', createdAt: 1000, updatedAt: 1000 }),
...
describe('load()', () => {
  ...
  it('populates providerConfigs from API response', async () => {
    const config: ProviderConfig = { id: 'pc-1', provider: 'openai', enabled: true, voiceType: 'api', defaultModel: 'gpt-4o', cliCommand: null, cliArgs: null, yoloMode: false, createdAt: 1000, updatedAt: 1000 };
...
describe('saveConfig()', () => {
  it('updates providerConfigs with saved value', async () => {
    stubPolyphonSettings();
    const config = { provider: 'anthropic', enabled: true, voiceType: 'api' as const, defaultModel: 'claude-opus-4-6', cliCommand: null, cliArgs: null, yoloMode: false };
  ...
  it('sets saveConfirmation to provider name', async () => {
    stubPolyphonSettings();
    const config = { provider: 'anthropic', enabled: true, voiceType: 'api' as const, defaultModel: null, cliCommand: null, cliArgs: null, yoloMode: false };
    await useSettingsStore.getState().saveConfig(config);

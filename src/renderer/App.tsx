  ...
  return (
    ...
    >
      ...
      <button
        ...
        title="Delete"
        className="p-1 rounded text-gray-400 hover:text-red-600 dark:hover:text-red-400 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors text-xs"
      >
        ✕
      </button>
    </div>
...
function Dashboard({
  ...
  async function handleCreate() {
    ...
    setCreating(true);
    try {
      const session = await window.polyphon.session.create(pickerComp.id, newName.trim());
      setSessions([session, ...sessions]);
      setShowPicker(false);
      setPickerComp(null);
      setNewName('');
          ...
          <>
            <div className="px-4 py-5 border-b border-gray-200 dark:border-gray-800 flex items-center justify-between">
              <button onClick={() => setView('home')} className="text-left">
                ...
                <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">One chat. Many minds.</p>
              </button>
              <button
